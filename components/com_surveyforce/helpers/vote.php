...
class SurveyforceVote {
    ...
    public static function checkSurveyStep($_survey_id, $_step) {
		...
		$survey_session = json_decode( $session->get('survey_step_'. $_survey_id) , true);
	...
	public static function setSurveyStep($_survey_id, $_step, $_init = false) {
		...
		if ($_init == true) {	// 初始化
			$session->clear('survey_step_'. $_survey_id);	// 先清空
		...
		$session->Set('survey_step_'. $_survey_id, json_encode($survey_session) );
