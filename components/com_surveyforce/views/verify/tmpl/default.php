...
<div class="survey_verify">
    ...
    <div class="verify">
        <form id="verify_form" name="verify_form" method="post" action="<?php echo JRoute::_('index.php?option=com_surveyforce&view=verify&task=verify.check_verify_form&Itemid=' . $this->itemid, false); ?>">
            <table class="formtable verify_info">
				<?php
				if ($this->verify_type) {
                ...
                <tr class="list">
                    <th rowspan="2">
                        <label for="recaptcha_response_field2">輸入驗證碼：</label>
                    </th>
                    <td>
                ...
                <tr>
                    <td>
                        <div id="captcha_field">
							<?php include_once(JPATH_SITE . "/rd/securimage/voice_show.php"); ?>
                        </div>
						<?php if ($this->preview == true) { ?>
                            <div class="preview_hover_captcha_field"></div>
						<?php } ?>
                    </td>
                </tr>
            </table>
            <div class="btns">
				...
				<?php } else { ?>
                    ...
                    <div>
                        <a href="<?php echo $this->preview == false ? $this->category_link : "javascript:void(0)"; ?>" class="btn ">取消</a>
                    </div>
...
<script>
    ...
    jQuery(document).ready(function () {
		<?php if ($level != 0) { ?>
        var td = '<th>驗證強度：</th>';
        td += '<td><div class="verifylevel_verify"><img src="' + '<?php echo JUri::root(); ?>' + 'images/system/VerifyLevel/verifylevel_<?php echo $level; ?>.svg" /></div></td>';
        var tr = '<tr>' + td + '</tr>';
        ...
        jQuery("#submit_img").bind("click", function () {
            ...
            jQuery.ajax({
                ...
                dataType: "json",
                data: {
                    'recaptcha_response_field': jQuery("#recaptcha_response_field2").val(),
                    'sid': jQuery("#sid").val()
                },
                beforeSend: function () {
        ...
        jQuery("#next_preview").bind("click", function () {
            ...
            jQuery(this).attr("href", url + "&type=" + "<?php echo $this->url_param; ?>");
