 ...
 */
class SurveyforceViewVerify extends JViewLegacy {
	...
	public function display($tpl = null) {
		...
		// 返回頁面連結
		if ($this->verify_required == 0 && count(json_decode($this->verify_type)) > 1) { // 若為擇一且有多個驗證方式
			$this->back_link = JRoute::_("index.php?option=com_surveyforce&view=verify_opt&sid={$this->survey_id}&Itemid={$this->itemid}", false);
		} else {
		...
		// 若為擇一選擇時，則驗證方式替換為先前的選擇
		if (SurveyforceVote::getSurveyData($this->survey_id, "opt_verify_types")) {
			$this->verify_type = json_encode(SurveyforceVote::getSurveyData($this->survey_id, "opt_verify_types"));
		}
