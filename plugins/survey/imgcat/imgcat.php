...
class plgSurveyImgcat {

	public function plgSurveyImgcat() {
		return true;

	}

	// 讀取分類清單
	public static function onGetAdminCats($_question_id) {
		$db = JFactory::getDBO();
		$query = $db->getQuery(true);

		$query->select('*');
		$query->from('#__survey_force_quests_cat');
		$query->where($db->quoteName('question_id') . " = '{$_question_id}'");
		...
		ob_start();
		include_once(JPATH_SITE . "/plugins/survey/imgcat/admin/html_cat.php");
		include_once(JPATH_SITE . "/plugins/survey/imgcat/admin/js_cat.php");
	...
	public static function onGetAdminOptions($_question_id) {
		...
		if ($cats) {
			...
			$query->select('c.title as ctext');
			$query->join('LEFT', '#__survey_force_quests_cat AS c ON c.id = a.catid');

			$db->setQuery($query);
			$rows = $db->loadObjectList();

			ob_start();
			include_once(JPATH_SITE . "/plugins/survey/imgcat/admin/html.php");
			include_once(JPATH_SITE . "/plugins/survey/imgcat/admin/js.php");
			$options = ob_get_contents();
			ob_clean();
		} else {
			$options = "請先儲存分類後，再繼續新增選項。";
		}
	...
	public function onSaveQuestion($_question_id) {
			...
			foreach ($cat_id as $key => $id) {
				...
				if ($id) { // 修改
					$fields = array (
						...
						$db->quoteName('title') . ' = ' . $db->quote($cat_text[$key]),
						$db->quoteName('ordering') . ' = ' . $db->quote($cat_order[$key])
					);
					...
					$query->update($db->quoteName('#__survey_force_quests_cat'))->set($fields)->where($conditions);

					$db->setQuery($query);

					$db->execute();
				} else { // 新增
					$columns = array ('question_id', 'title', 'ordering');

					$values = array (
						$db->quote($_question_id),
						$db->quote($cat_text[$key]),
						$db->quote($cat_order[$key])
					);

					$query->insert($db->quoteName('#__survey_force_quests_cat'))->columns($db->quoteName($columns))->values(implode(',', $values));
		...
		// 刪除分類
		$del_cat_ids = $post["del_cat_ids"];
		if ($del_cat_ids) {
			$query = $db->getQuery(true);
			$conditions = array (
				$db->quoteName('question_id') . ' = ' . $db->quote($_question_id),
				$db->quoteName('id') . ' IN (' . $del_cat_ids . ')'
			);
			$query->delete($db->quoteName('#__survey_force_quests_cat'));
		...
		$option_catid = $post["option_catid"];
			...
			foreach ($option_id as $key => $id) {
				...
				if ($id) { // 修改
					$fields = array (
						...
						$db->quoteName('catid') . ' = ' . $db->quote($option_catid[$key])
					);
					...
					$columns = array ('quest_id', 'ftext', 'ordering', 'catid');

					$values = array (
						$db->quote($_question_id),
						$db->quote($option_ftext[$key]),
						$db->quote($option_order[$key]),
						$db->quote($option_catid[$key])
					);
				...
				if ($option_image[$key]) {
					...
					if ($new_folder != $old_folder) {
						...
						// 移動小圖
						$new_image_small = $new_folder . "/" . $id . "_image_s" . pathinfo($option_image[$key], PATHINFO_BASENAME);
						$source_image_small = str_replace("tmp/", "tmp/s", $option_image[$key]);
						JFile::move(JPATH_SITE . "/" . $source_image_small, JPATH_SITE . "/" . $new_image_small);
					} else {
	...
	// 前台讀取選項表單與JS
	public static function onGetOptionsHtml($_question, $_options, $_cats, $_sub_options = null) {

		ob_start();
		include_once(JPATH_SITE . "/plugins/survey/imgcat/site/html.php");
		include_once(JPATH_SITE . "/plugins/survey/imgcat/site/js.php");
