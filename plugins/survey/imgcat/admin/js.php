...
<script type="text/javascript">
	...
	jQuery(document).ready(function () {
		...
		jQuery("#add_btn").bind("click", function () {
			...
			if(jQuery("#new_ftext").val().len() > 5){
                jQuery("#message_area").showMessage("新增選項 - 選項名稱的文字過多，請刪除部分文字。", jQuery("#new_ftext"));
                return false;
            }

			var num = jQuery('.option_ftext').length;
			for (var i = 0; i < num; i++) {
				if (jQuery("#new_ftext").val() == jQuery('.option_ftext')[i].value) {
			...
			jQuery.ajax({
				url: "../plugins/survey/imgcat/admin/ajax_upload_image.php",
				type: "POST",
				dataType: "json",
				data: formData,
				cache: false,
				...
				success: function (result) {
					...
					if (result.status == false) {
                        ...
                        content += '</td>';
                        content += '<td>';
                        content += jQuery("#new_catid").find(":selected").text();
                        content += '<input type="hidden" class="option_ctext" name="option_ctext[]" value="' + jQuery("#new_catid").find(":selected").text() + '"/>';
                        content += '<input type="hidden" class="option_catid" name="option_catid[]" value="' + jQuery("#new_catid").find(":selected").val() + '"/>';
                        content += '</td>';
                        content += '<td>';
		...
		jQuery(document).on("click", '.edit_row', function (e) {
            ...
            var character = 5 - jQuery("#new_ftext").val().len();
            jQuery("#ftext_char").html(character);

			option_catid = jQuery(this).parent().parent().children("td").children(".option_catid").val();
			jQuery("#new_catid").attr("value", option_catid);
		...
		jQuery("#edit_btn").bind("click", function () {
			...
			var image_content = jQuery("#table_list tr").eq(temp_tr_index).children("td").eq(3).html();
		...
		jQuery.fn.checkField = function () {
			...
			if (parseInt(jQuery('#jform_is_multi').val()) == 1) {
				...
				if (jQuery(".option_id").length < _min_options) {
					jQuery("#message_area").showMessage("複選類別 - 請至少新增" + _min_options + "個選項。", jQuery("#new_ftext, #text_upload_image, #new_catid"));
					return false;
				}
			} else {
				if (jQuery(".option_id").length == 0) {
					jQuery("#message_area").showMessage("單選類別 - 請至少新增1個選項。", jQuery("#new_ftext, #text_upload_image, #new_catid"));
                    return false;
				}
			}
        ...
        var character = 5 - new_ftext.val().len();
        if (character > 0) {
            jQuery("#ftext_char").html(character);
        } else {
        ...
        new_ftext.keydown(function () {
            jQuery(this).check(this, "#ftext_char", "選項名稱", 5);
        });
