...
<?php } ?>
				...
				if (stamp_num != 0) {
					for (var x = 0; x < jQuery(".stamp.fl-l.active").length; x++) {
					    var num = jQuery(".stamp.fl-l.active").parent().children(".intro").children("a").children("span")[x].innerHTML;
                        if (x > 0) {
							var cat = jQuery(".stamp.fl-l.active").parent().children(".intro").children("a").children("input")[x].value;
							jQuery(".option_active")[0].innerHTML += "、" + num[0] + "(" + cat + ")";
						} else {
							var cat = jQuery(".stamp.fl-l.active").parent().children(".intro").children("a").children("input")[x].value;
							jQuery(".option_active")[0].innerHTML = num[0] + "(" + cat + ")";
						}
...
<?php } ?>
		...
		jQuery(".question .tabs .tab").each(function (key) {
			jQuery('.question .tabs .tab_' + key).click(function () {
				jQuery('.question .tabs .tab').removeClass('active');
				jQuery(this).closest('.tab').addClass('active');
				jQuery('.question .tab_intro .tab_content').hide();
				jQuery('.question .tab_intro .tab_content_' + key).show();
			});

		});

		jQuery('.question .tab_intro .tab_content').hide();
		jQuery('.question .tab_intro .tab_content:first-child').show();
