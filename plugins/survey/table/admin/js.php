	...
	jQuery(document).ready(function () {
		...
		jQuery("#add_sub_btn").bind("click", function () {
			...
			if (jQuery("#new_sub_title").val() == "") {
                jQuery("#message_area").showMessage("新增選項 - 請填寫子選項名稱。", jQuery("#new_sub_title"));
				return false;
			}

            if (jQuery("#new_sub_title").val().len() > 25) {
                jQuery("#message_area").showMessage("新增選項 - 子選項名稱的文字過多，請刪除部分文字。", jQuery("#new_sub_title"));
                return false;
            }
            ...
            for (var i = 0; i < num; i++) {
                if (jQuery("#new_sub_title").val() == jQuery("#sub_table_list td[align='left']")[i].innerText) {
                    jQuery("#message_area").showMessage("新增選項 - 子選項名稱重複。", jQuery("#new_sub_title"));
		...
		jQuery("#edit_sub_btn").bind("click", function () {
			if (jQuery("#new_sub_title").val() == "") {
                jQuery("#message_area").showMessage("編輯選項 - 請填寫子選項名稱。", jQuery("#new_sub_title"));
				return false;
			}

            if (jQuery("#new_sub_title").val().len() > 25) {
                jQuery("#message_area").showMessage("編輯選項 - 子選項名稱的文字過多，請刪除部分文字。", jQuery("#new_sub_title"));
                return false;
            }
            ...
            for (var i = 0; i < num; i++) {
                if (jQuery("#new_sub_title").val() == jQuery("#sub_table_list td[align='left']")[i].innerText && jQuery("#sub_old_ftext").val() != jQuery("#new_sub_title").val()) {
                    jQuery("#message_area").showMessage("編輯選項 - 子選項名稱重複。", jQuery("#new_sub_title"));
                    jQuery("#new_sub_title").val("");
                    return false;
                }
            }
		...
		jQuery("#cancel_sub_btn").bind("click", function () {
			...
			jQuery("#new_sub_table .title").html("新增子選項");
		...
		jQuery.fn.checkField = function () {
			...
			if (jQuery(".sub_option_id").length == 0) {
				jQuery("#message_area").showMessage("請至少新增1個子選項。", jQuery("#new_sub_title"));
                return false;
			}
        ...
        var sub_character = 25 - new_sub_title.val().len();
        if (sub_character > 0) {
            jQuery("#sub_title_char").html(sub_character);
        } else {
            jQuery("#sub_title_char").html(0);
        }

        new_sub_title.keydown(function () {
            jQuery(this).check(this, "#sub_title_char", "子選項名稱");
        });
