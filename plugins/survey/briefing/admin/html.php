...
<table class="table table-striped" id="text_table">
	<thead>
		<tr>
			...
			<th width="100px">代表圖</th>
			<th width="100px">簡報檔</th>
	...
	<tbody id="table_list">
		...
		foreach ($rows as $frow) {
			...
			<tr>
				...
				<td>
					<?php if ($frow->file1) { ?>
						<a href="../<?php echo $frow->file1; ?>" target="_blank" title="檢視下載">檢視下載</a>
					<?php } ?>
					<input type="hidden" class="option_file1" name="option_file1[]" value="<?php echo $frow->file1; ?>"/>
				</td>
...
<div id="new_table" style="text-align:left;  ">
	...
	<table border="1" class="edit-tbl" >
		...
		<tr>
			<td>簡報檔*</td>
			<td>
				<div id="old_file_area">
					<a id="old_file_link" href="" target="_blank" title="檢視下載">檢視下載</a>&nbsp;&nbsp;
					<input class="btn" type="button" id="del_file_btn" style="width:70px " value="刪除檔案">
				</div>
				<input type="file" id="text_upload_file" name="text_upload_file" onchange="match_file(this.value);" >
				<br>
				*請上傳簡報檔(ppt/pptx/pdf)，最大容量為10MB。
			</td>
		</tr>
