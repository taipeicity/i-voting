            ...
            if (jQuery("#is_new_sub_option").val() == 1) {	// 未有
                ...
                if (_min_score < _max_score) {
                    ...
                    if (_option_count > _diff_score) {
                        jQuery("#message_area").showMessage("選項數目需小於等於分數設定值。", jQuery("#number_min_score"));
                        return false;
                    }
                } else {
                    jQuery("#message_area").showMessage("分數設定需最高分大於最低分。", jQuery("#new_scroe select"));
                    return false;
                }
            } else {
                ...
                _diff_score = _max_score - _min_score + 1;
                if (_option_count > _diff_score) {
                    jQuery("#message_area").showMessage("選項數目需小於等於分數設定00值", jQuery("#number_min_score"));
                    return false;
                }
            }
        };
