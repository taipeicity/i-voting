    ...
    jQuery(document).ready(function () {
        ...
        jQuery("#add_btn").bind("click", function () {
            ...
            if (jQuery("#text_upload_image").val() == "") {
                jQuery("#message_area").showMessage("新增選項 - 請選擇圖片。", jQuery("#text_upload_image"));
                return false;
            }

            if (jQuery("#new_desc").val() == "") {
                jQuery("#message_area").showMessage("新增選項 - 請填寫描述。", jQuery("#new_desc"));
                return false;
            }

            if (jQuery("#new_desc").val().len() > 25) {
                jQuery("#message_area").showMessage("新增選項 - 選項名稱的文字過多，請刪除部分文字。", jQuery("#new_desc"));
                return false;
            }
            ...
            jQuery.ajax({
                url: "../plugins/survey/imgtext/admin/ajax_upload_image.php",
                type: "POST",
                dataType: "json",
                data: formData,
                cache: false,
            ...
            if (image_content != '') {
                ...
                content += '<input type="hidden" class="option_desc" name="option_desc[]" value="' + jQuery("#new_desc").val() + '"/>';
        ...
        jQuery(document).on("click", '.edit_row', function (e) {
            ...
            option_desc = jQuery(this).parent().parent().children("td").children(".option_desc").val();
            jQuery("#new_desc").val(option_desc);

            var desc_character = 25 - jQuery("#new_desc").val().len();
            jQuery("#desc_char").html(desc_character);
        ...
        jQuery("#edit_btn").bind("click", function () {
            ...
            if (jQuery("#new_desc").val() == "") {
                jQuery("#message_area").showMessage("編輯選項 - 請填寫描述。", jQuery("#new_desc"));
                return false;
            }

            if (jQuery("#new_desc").val().len() > 25) {
                jQuery("#message_area").showMessage("編輯選項 - 描述的文字過多，請刪除部分文字。", jQuery("#new_desc"));
                return false;
            }
            ...
            if (jQuery("#old_image_link").attr("href") == "") {
                if (jQuery("#text_upload_image").val()) {
                    ...
                    jQuery("#message_area").showMessage("編輯圖片 - 請選擇圖片。", jQuery("#text_upload_image"));
                    return false;
                }
            }
        ...
        jQuery.fn.checkField = function () {
            ...
            if (parseInt(jQuery('#jform_is_multi').val()) == 1) {
                ...
                if (jQuery(".option_id").length < _min_options) {
                    jQuery("#message_area").showMessage("複選類別 - 請至少新增" + _min_options + "個選項。", jQuery("#new_ftext, #text_upload_image, #new_desc"));
                    return false;
                }
            } else {
                if (jQuery(".option_id").length == 0) {
                    jQuery("#message_area").showMessage("單選類別 - 請至少新增1個選項。", jQuery("#new_ftext, #text_upload_image, #new_desc"));
                    return false;
                }
            }
        ...
        var new_desc = jQuery("#new_desc");
        var desc_character = 25 - new_desc.val().len();
        if (character > 0) {
            jQuery("#desc_char").html(desc_character);
        } else {
            jQuery("#desc_char").html(0);
        }

        new_desc.keydown(function () {
            jQuery(this).check(this, "#desc_char", "描述");
        });

        new_desc.keyup(function () {
            jQuery(this).check(this, "#desc_char", "描述");
        });

        new_desc.keypress(function () {
            jQuery(this).check(this, "#desc_char", "描述");
        });
