...
class SurveyforceModelQuestions extends JModelList {
	...
	function delete($cid) {
		...
		foreach ($cid as $question_id) {
			...
			$query = $db->getQuery(true);
			$query->select('sc.*');
			$query->from($db->qn('#__survey_force_vote_count', 'c'));
			$query->leftJoin($db->qn('#__survey_force_vote_sub_count', 'sc') . ' ON sc.field_id = c.field_id');
			$query->where('c.question_id = ' . $db->q($question_id));
			$db->setQuery($query);
			$sub_fields = $db->loadObjectList();

			foreach ($sub_fields as $sub_field) {
				$query = $db->getQuery(true);
				$query->delete('#__survey_force_vote_sub_count');
				$query->where('id = ' . $db->q($sub_field->id));
				$db->setQuery($query);
				$db->execute();
			}
			...
			// 刪除票數統計
			$query = $db->getQuery(true);
			$query->delete('#__survey_force_vote_count');
			$query->where('question_id = ' . $db->quote($question_id));
			$db->setQuery($query);
			$db->execute();
		}
	...
	public function getSections() {
		...
		if ($survid) {
			$query = " SELECT a.* " . " FROM #__survey_force_qsections AS a " . " WHERE 1=1 " . ($survid ? "\n AND a.sf_survey_id = $survid" : '') . " ORDER BY a.ordering";
			$database->setQuery($query);
