...
class SurveyforceModelSurveys extends JModelList {
    ...
    protected function populateState($ordering = null, $direction = null) {
		...
		// $unit = $this->getUserStateFromRequest($this->context . '.filter.unit', 'filter_unit', '', 'string');
		// $this->setState('filter.unit', $unit);
    ...
    protected function getListQuery() {
        ...
        $query->select('s.`id`, s.`title`, s.`is_public`, s.`published`, s.is_complete, s.is_checked, s.publish_up, s.publish_down, s.vote_start, s.vote_end, s.created_by, s.is_lottery, s.is_place');
		...
		// filter 課室單位		
		// $unit = $this->getState('filter.unit');
		// if (is_numeric($unit)) {
			// $query->where('u.unit_id= ' . (int) $unit);
		// }
