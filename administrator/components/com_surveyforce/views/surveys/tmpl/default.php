...
<script type="text/javascript">
    ...
    Joomla.submitbutton = function (task) {
		...
		// 議題數據分析按鈕
		if (task == 'surveys.go_count') {
			console.log(task);
            location.href = "index.php?option=com_surveyforce&view=surveys_count";
			
			return false;
		}
		
		// 議題流量分析
		if (task == 'surveys.go_graffic') {
            location.href = "index.php?option=com_surveyforce&view=surveys_traffic";
    ...
    <div id="j-main-container" class="span10">
        ...
        <table class="table table-striped" id="testimonialsList" style="min-width:1100px;">
            ...
            <tr>
                ...
                <th class="hidden-phone checkbox-width">
                    <label>
                ...
                <th class="nowrap center status-width">
                    流程狀態
                </th>

                <th class="nowrap center stage-width">
                    階段
                </th>
                ...
                <tr class="row<?php echo $i % 2; ?>" sortable-group-id="1">
                    ...
                    <td class="nowrap center">
                        <div style="white-space: nowrap; text-overflow: ellipsis; overflow: hidden; width:200px;">
                            <?php if ($canEdit) : ?>
                                <a class="hasTooltip"
                                   href="<?php echo JRoute::_('index.php?option=com_surveyforce&view=survey&layout=edit&id='.$item->id); ?>"
                    ...
                    <td class="center">
                        <?php
                        if (array_key_exists($item->id, $this->totalnum)) {
                            echo $this->totalnum[$item->id]->num + $item->paper_total_vote;
                        } else {
                            if ($item->stage > 5) {
								if ($item->paper_total_vote) {
									echo $item->paper_total_vote;
								} else {
									echo '無投票資料';
								}
                            } else {
                    ...
                    <?php if (in_array(8, $self_gps)) { ?>
                        <td class="center">
                            <a href="/administrator/index.php?option=com_surveyforce&task=surveys.isTest&id=<?php echo $item->id; ?>"
                               type="button"
                               class="is-test<?php echo $is_processing ? "" : " disabled"; ?>"><?php echo
                                $item->isTest ? '<i 
                               class="fas 
                               fa-check"></i>' :
                                    '<i class="fas 
                        fa-times"></i>'; ?></a>
                            <input type="hidden" value="<?php echo $item->isTest; ?>"/>
                        </td>
                    <?php } ?>
                    ...
                    <td class="nowrap">
                        ...
                        if ($item->created_by == $userId || ($item->unit_id == $unit_id && $this->canDo->get('core.review')) || $this->canDo->get('core.own')) {
                            ?>
                            <div class="center">
                                <?php
                                unset($funs);
                                ...
                                <div style="display: none;" id="survey_preview<?php echo $item->id; ?>"
                                     itemid="<?php echo $item->id; ?>" tabindex="-1"
                                     class="modal hide fade survey_preview">
                                    ...
                                    <div class="modal-footer">
                                        <button type="button" class="btn" data-dismiss="modal" aria-hidden="true">關閉
                                        </button>
                                    </div>
                                ...
                                if ($item->is_checked) {
                                    ...
                                    if ($item->release_stage > 5) {
                                        ...
                                        $voting_item = $menu->getItem($this->voting_itemid)->title;
                                        $complete_item = $menu->getItem($this->complete_itemid)->title;
                                        $practice_item = $menu->getItem($this->practice_itemid)->title;

                                        if ($item->is_public == 0) {
                                            $url = JRoute::_($voting_item.DS.$item->id.'-survey-intro'.'?token='.md5($item->id), false);
                                            $url_prac = JRoute::_($practice_item.DS.$item->id.'-survey-intro'.'?token='.md5($item->id), false);
                                            $url_result = JRoute::_($complete_item.DS.$item->id.'-survey-result'.'?token='.md5($item->id), false);
                                        } else {
                                            $url = JRoute::_($voting_item.DS.$item->id.'-survey-intro', false);
                                            $url_prac = JRoute::_($practice_item.DS.$item->id.'-survey-intro', false);
                                            $url_result = JRoute::_($complete_item.DS.$item->id.'-survey-result', false);
                                        }

                                        ?>
                                        <a href="#divFormLink<?php echo $item->id; ?>"
                                           id="btnFormLink<?php echo $item->id; ?>" title="投票網址" style="display:none">投票網址</a>
                                        <div id="divFormLink<?php echo $item->id; ?>" style="display:none;">
                                            <?php if ($item->vote_pattern != 2) { ?>
                                                <div>
                                                    正式連結：<input type="text" title="正式連結"
                                                                value="<?php echo JUri::root().$url; ?>" size="50"
                                            ...
                                            <?php if ($item->vote_pattern != 1) { ?>
                                                <div>
                                                    練習連結：<input type="text" title="練習連結"
                                                                value="<?php echo JUri::root().$url_prac; ?>" size="50"
                                                                style="width:350px;"><br>
                                                    <a href="<?php echo '/'.$url_prac; ?>" target="_blank"
                                                       title="點此開啟前台頁面">點此開啟練習頁面</a>
                                                </div>
                                            <?php } ?>
                                            <?php if (($item->display_result == 2 && strtotime(JHtml::_('date', $item->vote_end, JText::_('DATE_FORMAT_LC5'))) < time()) || $item->display_result == 1) { ?>
                                                <div>
                                                    結果連結：<input type="text" title="結果連結"
                                                                value="<?php echo JUri::root().$url_result; ?>"
                                                                size="50" style="width:350px;">
                                                    <br>
                                                    <a href="<?php echo '/'.$url_result; ?>" target="_blank"
                                                       title="點此開啟結果頁面">點此開啟結果頁面</a>
                                                </div>
                                            <?php } ?>
                                        </div>
                                    ...
                                    // 開始投票後才會有資料
                                    if ($is_processing) {
                                        $funs[] = '<a class="hasTooltip" href="index.php?option=com_surveyforce&view=result&id='.$item->id.'" title="觀看結果">觀看結果</a>';
                            ...
                            if (in_array($this->export_result, $user->groups) || in_array($this->print, $user->groups) || in_array($this->show_result, $user->groups)) {
                                ?>
                                <div class="center">
                                    <?php
                                    unset($funs);
                                ...
                                if (($item->unit_id == $unit_id && in_array($this->undertaker, $self_gps)) || ($item->unit_id != $unit_id && $cross_unit == 1)) {
                                    ?>
                                    <div class="center">
                                        <?php
                                        unset($funs);
