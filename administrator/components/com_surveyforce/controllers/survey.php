	...
	protected function postSaveHook(JModelLegacy $model, $validData = array()) {
		...
		if ($ivoting_path) {
			...
			foreach ($upload_pdfs as $i => $upload_pdf) {
				if ($upload_pdf["name"] && $upload_pdf["tmp_name"]) {
					if ($upload_pdf["error"] != 0) {
						if (!$upload_pdf["old_pdf"]) {
							$result_pdf["msg"] = "上傳檔案失敗。";
						}
					}

					// 檢查檔案大小
					if ($upload_pdf["size"] > 5242880) {
						$result_pdf["msg"] = "上傳檔超過指定大小(5MB)。";
					}

					// 檢查副檔名
					$allow_files = array("application/pdf");
					if (!in_array($upload_pdf["type"], $allow_files)) {
						$result_pdf["msg"] = "只允許上傳PDF。";
					}
